...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, openURLContexts URLContexts: Set<UIOpenURLContext>) {
        ...
        if url.scheme == "paytmmp" || url.scheme == "paytmuniqueid" {
        ...
        }  else if url.scheme == "mspmp" {
            if let navigationController = window?.rootViewController as? UINavigationController {
                if let viewController = navigationController.viewControllers.first as? ModeSelectionViewController {
                    guard let _ = dict["am"] else {
                        return
                    }
                    navigationController.setViewControllers([viewController], animated: false)
                    viewController.updateVariablesForMscanSampleAppInvoke(dict: dict)
                }
            }
        }
    }
